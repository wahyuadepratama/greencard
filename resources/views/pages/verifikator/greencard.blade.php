...
<div id="greencard-webview">
  ...
  <div id="close-status" class="tabcontent" style="border: 1px solid #ddd; border-top:0px;">
        ...
        <table id="close-table-data" class="table table-striped table-bordered" style="width:100%">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>
                @if($mClose->user->section_id == session('login')->section_id)
    ...
    @include('mobile-views.verifikator.greencard')
</div>
@endsection
  ...
  <script type="text/javascript">
    ...
    function searchSectionData(){
      ...
      $.ajax({ /* THEN THE AJAX CALL */
        url: "/verifikator/greencard/open/data/search",
        method : "POST",
      ...
      $.ajax({ /* THEN THE AJAX CALL */
        url: "/verifikator/greencard/close/data/search",
        method : "POST",
        data:{'section':sectionChoose, 'year': sectionYear, _token: '{{csrf_token()}}'},
        async : true,
        dataType : 'text',
      ...
      $.ajax({ /* THEN THE AJAX CALL */
        url: "/verifikator/greencard/open/data/mobile/search",
        method : "POST",
        data:{'section':sectionChoose, 'year': sectionYear, _token: '{{csrf_token()}}'},
        async : true,
        dataType : 'text',
      ...
      $.ajax({ /* THEN THE AJAX CALL */
        url: "/verifikator/greencard/close/data/mobile/search",
        method : "POST",
        data:{'section':sectionChoose, 'year': sectionYear, _token: '{{csrf_token()}}'},
        async : true,
        dataType : 'text',
    ...
    function loadModal(id) {
      ...
      function storeNewStatus(id, status){
        $.ajax({ /* THEN THE AJAX CALL */
          url: "/verifikator/greencard/change-status",
          method : "POST",
